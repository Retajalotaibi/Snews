  ...
  return (
    <div className="space-y-9">
      ...
      {newsQuery.isLoading ? <QueryLoading label="Scanning the newswire" /> : newsQuery.isError ? <QueryError onRetry={() => newsQuery.refetch()} label="The news feed is temporarily unavailable." /> : !response || !response.articles?.length ? <QueryEmpty label="No current headlines match this desk." /> : (
        <section>
           <div className="mb-5 flex items-center justify-between"><p className="font-data text-[10px] uppercase tracking-[.16em] text-muted-foreground">{response.articles.length} market-moving stories in view</p><p className="hidden items-center gap-1 text-xs text-muted-foreground sm:flex"><Clock3 size={13} /> Plain-English impact, not predictions</p></div>
